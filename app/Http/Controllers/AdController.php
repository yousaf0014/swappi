...
class AdController extends Controller
{
	...
	public function save(Request $request)
	{
		...
		$data['pkg_price'] = $this->get_package_price($request->package);		 
		$data['image_price'] = $request->input('extra_images_price');
		//Send Email notification to admin
		Mail::send('admin.emails.new_ad', $data, function ($message) use ($data) {
			$message->from(config('constants.NO_REPLY_EMAIL'))->bcc($data['email'])
			->to(config('constants.ADMIN_EMAIL'))
			->subject('Din nye annonce på Swappi');
	...
	/******
	*** payemnt for paypal and mobilepay in popupwindow
	***/

	public function iframe($orderID, $amount,$method){
		$paymentController = new PaymentController();
		$orderID = Auth::user()->id.'-'.Carbon::now()->timestamp;
		$params = array(
			"version"      => "v10",
			"merchant_id"  => 16544,
			"agreement_id" => 145565,
			...
			"continueurl" => url('/').'/success/'.$orderID,
			"cancelurl"   => url('/').'/cancel/'.$orderID,
			"callbackurl" => url('/').'/callback'
		);

		$checksum = $paymentController->sign($params);
	...
	public function success($orderid,Request $request){
		$paymentController = new PaymentController();
		$paymentData = $paymentController->get_payment_id($orderid);
		$payment_id = 0;
		if (isset($paymentData[0]) && $paymentData[0]->accepted == true) {
			$payment_id = $paymentData[0]->id;
		}
		return  view('ad.success',['order_id'=>$orderid,'payment_id'=>$payment_id]);
	}
	public function cancel($orderid,Request $request){
		return  view('ad.cancel');
	}
	...
	function get_order_details($ad_id, $order_id){
		...
		if (isset($details[0]) && $details[0]->accepted == true) {
			...
			Mail::send('admin.emails.new_ad', $data, function ($message) use ($data) {
				...
				$message->bcc(Auth::user()->email)->subject('Swappi ny annonce Ankom');
			});
