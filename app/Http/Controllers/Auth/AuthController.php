...

    {
    	...
    	//return User::create($userData);
    	        ...
    	        Mail::send('admin.emails.new_user', $userData, function ($message) use ($data) {
    	        	$message->from(config('constants.NO_REPLY_EMAIL'))->bcc(config('constants.NO_REPLY_EMAIL'))
                    ->to(config('constants.ADMIN_EMAIL'))
                    ->subject('Swappi New User Registered');


    	        });

                Mail::send('admin.emails.new_user_confirm', $userData, function ($message) use ($data) {
                    $message->from(config('constants.NO_REPLY_EMAIL'))->to($data['email'])
                    ->subject('Swappi Confirm User Registered');
                });
