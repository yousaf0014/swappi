...

{
	...
	public function index()
	{
		...
		//dd(DB::getQueryLog());
		$firstLogin = \Session::pull('first_login',0); 
...

	{
		...
		$users = User::orderby('created_timestamp', 'DESC')
	...
	public function experienceSave(Request $request) {
		...
		if($request->input('edi') != ''){
			$data['end_date_identifier'] = $request->input('edi');
		} 

		if($request->input('edi_new') != ''){
			$data['end_date_identifier'] = $request->input('edi_new');
		} 

		if($request->input('endDate') != ''){

			$data['endDate'] = changeToTimestamp1($request->input('endDate'));
			...
			if($conection > 0){
				...
				if($request->ajax()){
					...
					return json_encode([
						...
						'msg'	=> 'Allerede forbundet..'

					]);

				}else{

					$request->session()->flash('success', 'Allerede forbundet..');
			...
			if(Connection::insert($data)){
				...
				if($request->ajax()){
					...
					return json_encode([
						...
						'msg'	=> 'Anmodning afsendt.'

					]);

				}else{

					$request->session()->flash('success', 'Anmodning afsendt.');
