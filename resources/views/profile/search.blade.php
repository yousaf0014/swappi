     ...
     <div class="row">
      ...
      <div class="col-md-12 col-sm-col-xs-12">
       <div class="col-md-3 col-sm-3 col-xs-12">
       	<form action="{{ url()->current() }}" method="get" id="profile-filter">
	        <div class="rune_peterson_profile_find">
	         <p>
	         <!--	<img class="pull-left img-circle" width="61" src="{{ profile_image_link(Auth::user()->profilePic, 61, 61) }}">
...
@section('scripts')
	<script type="text/javascript">
	 	...
	 	$(document).ready(function(){
			...
			//Make ajax call
			$.ajax({
				url: "{{ url('profile/get_search_users') }}",
				method: "GET",
				headers: {'X-CSRF-TOKEN': token},
				@if(app('request')->input('find') == 'profile')
					data: $('#profile-filter').serialize() + '&find=profile'
				@endif
			}).done(function(data) {
				$('#see-more-users').attr('data-currentpage', '2');
				if(data.indexOf("false") > -1){
					$('#see-more-users').hide();
					$('#users-con tbody').html('<tr><td align="center" colspan="5">No users to show.</td></tr>');
				}else{
			...
			$('#see-more-users').on('click', function(){
				...
				//Make ajax call
				$.ajax({
					url: "{{ url('profile/get_search_users?page=') }}"+current,
					method: "GET",
					headers: {'X-CSRF-TOKEN': token},
					@if(app('request')->input('find') == 'profile')
						data: $('#profile-filter').serialize() + '&find=profile'
		...
		$('#profile-filter').on('submit', function(e){
			if($('#profile-filter input[name=firstName]').val() == ''){
				$('.error-rm').remove();
				$('input[name=firstName]').focus();
				$('#profile-filter input[name=firstName]').closest('div').append('<p class="error-rm text-danger"><i class="fa fa-times"></i> Indtast nogle søgeord</p>');
				e.preventDefault();
			}
		});
	</script>
