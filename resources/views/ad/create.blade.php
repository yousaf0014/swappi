...
<body>
		          ...
		          <div id="step-4" class="form-step">
		          	<div id="step4-inner">
			        	...
			        	<div class="Hvad_søger_du_section paid-field">
				            <h2 style="text-decoration: underline; ">Din annonces samlede pris: <span style="text-decoration: underline; " id="final_amount" class="pull-right"></span></h2>
				           <!-- <div class="payment_method_option_stepfour">
				            ...
				            <div class="col-md-12 col-sm-12 col-xs-12 email_add_telefornne">
				             ...
				             <div class="card">
						      	<input class="pay" type="text" autocomplete="off" required >
						     </div>
				            </div>
				            <div class="col-md-12 col-sm-12 col-xs-12 email_add_telefornne Kortnummer_email">
				              ...
				              <div class="col-md-6 col-sm-6 col-xs-12">
				               ...
				               <div class="card">
				              	<div class="card-brand"></div>
				               	<input class="pay" type="text" required data-quickpay="cardnumber" autocomplete="off" >
				               	</div>
				              </div>
							 ...
							 <div class="col-md-6 col-sm-6 col-xs-12 Telefonnummer_tele">
				              ...
				              <div class="col-md-3 col-lg-3 col-sm-12 col-xs-12 email_add_telefornne">
									...
									<input class="pay" required type="password" required="" maxlength="4" autocomplete="off" data-quickpay="cvd">
								</div>
					...
					<div class="MobilePay_pal" id="p_gateways">
						<div class="">
							<button type="button" onclick="openpaymentwindow('mobilepay')" name="previous" class="col-md-6 col-sm-6 col-xs-12 payment_method_option_stepfour_left"><img src="{{ asset('images/mobilepay.png') }}">MobilePay</button>
			            </div>
			            <div class="" style="display:none;">
			            	<button type="button" onclick="openpaymentwindow('paypal')" name="previous" class="col-md-6 col-sm-6 col-xs-12 payment_method_option_stepfour_right">
	...
	<div style="display:none" id="iframediv">
		<a href="{{url('ad/iframe')}}" id="a_iframe" class="glyphicon glyphicon-edit edit_info"></a>
	</div>
	<style>
		.btn-custom{
    ...
    <script type="text/javascript">
		...
		$('.next').click(function(){
			...
			if (true){
				...
				if ($('#step-1').is(":visible")){
					...
					if (!$("input[name='skills']:checked").val()) {
						flag = false;
						alert('Du skal udfylde dette felt');
					}else{
						current_fs = $('#step-1');
						next_fs = $('#step-2');
					}
		...
		$('.next.paylink').on('click', function(){
			...
			$.ajax({
				...
				if(values.status == true){
					...
					var amont = parseInt(values.data.total_amount)/100;

					$('#final_amount').html(parseInt(values.data.total_amount)/100+ ' kr.');
					var hrf = $('a#a_iframe').attr('href');
					hrf += '/'+values.data.order_id + '/'+values.data.total_amount;
					$('a#a_iframe').attr('href',hrf);
					if(amont > 0){
						$('#p_gateways').show();
					}else{
						$('#p_gateways').hide();
		...
		function openpaymentwindow(method){
			var hrf = $('a#a_iframe').attr('href');
			hrf += '/'+ method
			window.open(hrf, "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=500,left=500,width=430,height=700");
		}
