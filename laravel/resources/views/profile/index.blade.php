         ...
         <div class="Rune_Petersen_profile_section">
          ...
          <div class="Digital_designer_section">
            @if(count($user[0]->experiences) > 0)
            	<h2>Titel : {{ $user[0]->jobTitle }}</h3>
            	<!-- <h3>{{ $user[0]->description }}</h3> -->
            @endif
            <br />
            @if(count($user[0]->experiences) > 0)
	            	<h4 class="user-orgname">
		...
		<?php //TODO Compounds(Forbindelser) section ?>     
		    ...
		    <!--   Endorsment Section Start -->
				...
				<div id="kompetencer" class="ui-state-default kompetencer_section"> 
				     <h1>
				    	<img src="{{ asset('images/kompetencer.png') }}">Kompetencer
				    </h1> 
				    	<div class="Forbindelser_inner_section">
				    		@foreach($endosrmentsSkills as $skID=>$skName)
								<?php $endCount = !empty($userEndorsments[$skID]) ? count($userEndorsments[$skID]):0; ?>
								<div class="col-md-12" id="row{{$skID}}">									
									<div class="col-md-6 col-sm-6">
									    @if (Auth::check() && $canEndorse == true ) 
									    		<a href="javascript:;" onclick="sendRecomendationUser('{{$skID}}','{{$user[0]->id}}',this);">
									    @endif
									    <div class="col-md-2 col-sm-2 kom_span_count" style="float: left;font-weight:bold;">
									    	{{ $endCount }}
									    </div>
									    @if (Auth::check() && $canEndorse == true ) 
									    	</a>
									    @endif
									    <div class="col-md-10 col-sm-10" style="float: left;padding:0px;">
									    @if (Auth::check() && $canEndorse == true ) 
									    	<a href="javascript:;" onclick="sendRecomendationUser('{{$skID}}','{{$user[0]->id}}',this);">
									    @else
									    <div style="float: left;">
									    @endif
									    	<div class="kom_span_cat" style="float:left;font-weight:normal">{{$skName}}</div>									    	
									    @if (Auth::check() && $canEndorse == true ) 
									    	</a>
									    @else
									    </div>									    
									...
									<div class="col-md-6 col-sm-6">
							    		<div class="carousel slide" data-ride="carousel" data-type="multi" data-interval="3000" id="myCarousel{{$skID}}">
							                <div class="carousel-inner">
							                  	<div class="item active">
							                  		@if(!empty($userEndorsments[$skID]))
									                  	<?php $start = 1; $html = ''; ?>
									                  	@foreach($userEndorsments[$skID] as $usr)
									                  		{{ $start == 0 || ($start % 6) == 0 ? $html :''}}
									                    	<?php $html = '</div><div class="item">'; ?>
									                      	<?php $start++; ?>
									                      	<a href="{{url('/profile/user',$usr->id)}}" title="{{$usr->fName.' '.$usr->lName}}">
									                      		<img  style="float:right;width: 30px; height: 34px;" class="img-responsive" src="{{ profile_image_link($usr->profilePic, 30, 30) }}">
									                      	</a>
									                     @endforeach
								                     @endif
							                    </div>
							            ...
							            @if($endCount > 6)
					                  		<a style="margin-left: -6%;" class="left carousel-control" href="#myCarousel{{$skID}}" data-slide="prev"><img style="padding-top: 8px;" src="{{ asset('images/arrow.png') }}" height="20" width="40" /></a>
					                  		<a style="padding-left: 0%;" class="right carousel-control" href="#myCarousel{{$skID}}" data-slide="next"><img style="padding-top: 8px;" src="{{ asset('images/01.png') }}" height="20" width="40" /></a>
					                  	@endif
					            	</div>
				            	</div>				            	
								<div class="clearfix"></div>
					...
					<div class="col-md-12 col-sm-12" style="margin-top:15px">
						<div class="col-md-offset-5">
							<?php /* @if($userSkills->previousPageUrl())
							<a href="{{$userSkills->previousPageUrl()}}" style=" background: #e0e0dc none repeat scroll 0 0; padding: 8px 6px 6px;"> 
					        	<small>Vis dyrebare Kompetencer</small>
					        </a>
							@endif */ ?>

							@if($userSkills->nextPageUrl())
							<a href="{{$userSkills->nextPageUrl()}}" style=" background: #e0e0dc none repeat scroll 0 0; padding: 8px 6px 6px;"> 
					        	<small>Vis flere Kompetencer</small>
					        </a>
							@endif
						</div>
					</div>
...
@section('scripts')
	<script>
		 ...
		 function sendRecomendationUser(skillID,uID,elem){
			$.ajax({
		          url: "{{url('/profile/endorsmentd/save')}}",
		          type: "get",
		          data: {
		          	'skill_id':skillID,
		          	'user_id':uID
