	         	...
	         	<form id="ad-create" method="post" action="{{ url('ad/save') }}" enctype="multipart/form-data">
		          ...
		          <div id="step-1" class="form-step">
		          	...
		          	<div class="Hvad_søger_du_section">
						...
						<div class="step-pagination no-padding">
		...
		function ajax_form_submit(input, file){
			...
			$.ajax({
				...
				if(data.status == true){
					...
					alert('Billede ikke uploadet! Prøv igen');
    ...
    <script type="text/javascript">
		...
		$('.next').click(function(){
			...
			if (true){
				...
				var is_selected = $('[name="skills"]:checked').val();
				if(typeof(is_selected) == 'undefined'){
					$('.pad_create_Ad').prepend('<p style="width: 100%;" class="alert alert-danger selected_skill"><i class="fa fa-times"></i> Vælg venligst en færdighed</p>');
					return false;
				} else {
			...
			success: function(form, data) {
				...
				$.ajax({
					...
					if($('input[name=check]:checked').val() == 1){
						...
						alert('Accepter venligst vilkår og betingelser');
