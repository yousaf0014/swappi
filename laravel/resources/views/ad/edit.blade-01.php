	        ...
	        <div class="col-md-9 col-sm-9 col-xs-12 fotograph_ad_section_left">
	         ...
	         <div class="photograph_fhotoshoot">
	           ...
	           <ul>
		            ...
		            <li class="ad-created">
		            	<img src="{{ asset('images/operatted_icon.png') }}">Oprettet den {{ $ad->createdAt->format('d / m-Y') }}
		            	<a class="btn btn-link edit-icon" href="#" data-toggle="modal" data-target="#adcreated"><i class="fa fa-pencil"></i></a>
		            </li>
		            <li><?php // TODO Bookmarking for ad ?><img src="{{ asset('images/star_polygon.png') }}"></li>
	           </ul>
	           <!-- Ad Exchange Modal -->
	         ...
	         <div class="Beskrivelse_section">
	           ...
	           <h3><a href="{{ url('profile/' . $ad->user->username) }}">Kontakt udbyder</a></h3>
	           ...
	           @foreach($ads as $k => $ad)
		          <div class="col-md-4 col-sm-4 col-xs-12 Advokat_søges{{ $k+1 == count($ads) ? ' last_col_advocate' : '' }}">
		             <div class="Udbyder_star">
		             	<?php //TODO "<h5>Udbyder : Google</h5>" ?>
		             	<h5>Udbyder : Google</h5>
...
@section('scripts')
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$('#adheadline').on('hidden.bs.modal', function (e) {
				var data = $('#adheadline form').serialize();
			...
			$('#adexchange').on('hidden.bs.modal', function (e) {
				var data = $('#adexchange form').serialize();
				var token = $('[name="_token"]').val();
				$.ajax({
					url: "{{ url('ad/update') }}",
					method: "POST",
			...
			$('#adcreated').on('hidden.bs.modal', function (e) {
				var data = $('#adcreated form').serialize();
				var token = $('[name="_token"]').val();
				$.ajax({
					url: "{{ url('ad/update') }}",
					method: "POST",
			...
			$('#addescription').on('hidden.bs.modal', function (e) {
				var data = $('#addescription form').serialize();
				var token = $('[name="_token"]').val();
				$.ajax({
					url: "{{ url('ad/update') }}",
					method: "POST",
